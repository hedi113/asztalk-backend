...
var arthur = tudosok.Where(x => x.Keresztnev == "Arthur" && x.Vezeteknev == "B. McDonald").Select(x => x.Tipus);

Console.WriteLine($"3. feladat: {arthur}");

var irodalmi = tudosok.Where(x => x.Evszam == 2017 && x.Tipus == "irodalmi");

Console.WriteLine($"4. feladat: {irodalmi.Select(x => x.Keresztnev)} {irodalmi.Select(x => x.Vezeteknev)}");
